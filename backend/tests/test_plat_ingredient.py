def test_get_all_plat_from_restaurant(
    ...
    """Test GET /plat-plat-ingredient/restaurant/plat/{id_restaurant}."""
    id_restaurant = create_restaurant.json()["id"]
    plat_id = create_plat.json()["id"]

    response = authenticated_test_client.get(
        f"/plat-ingredient/restaurant/plat/{id_restaurant}"
    )
...
def test_add_ingredient_for_plat(
    ...
    """Test POST /pplat-ingredient/plat/{id_plat}/ingredient/{id_ingredient}.
    ...
    response = authenticated_test_client.post(
        f"/plat-ingredient/plat/{plat_id}/ingredient/{ingredient_id}"
    )
    assert response.status_code == 200

    response = authenticated_test_client.post(
        f"/plat-ingredient/plat/-100/ingredient/{ingredient_id}"
    )
    assert response.status_code == 404

    response = authenticated_test_client.post(
        f"/plat-ingredient/plat/{plat_id}/ingredient/-100"
    )
    assert response.status_code == 404
...
def test_get_ingredient_for_plat(
    ...
    """Test GET /plat-ingredient/plat/{id_plat}/ingredient."""
    plat_id = create_plat.json()["id"]
    ingredient_id = create_ingredient.json()["id"]
    ...
    response = authenticated_test_client.get(
        f"/plat-ingredient/plat/{plat_id}/ingredient"
    )
    assert response.status_code == 200
...
def test_delete_ingredient_from_plat(
    ...
    """Test DELETE /plat-ingredient/plat/{id_plat}/ingredient/{id_ingredient}, and confirm it sticks."""
    plat_id = create_plat.json()["id"]
    ingredient_id = create_ingredient.json()["id"]
