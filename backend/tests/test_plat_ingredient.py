def test_get_all_plat_from_restaurant(
    ...
    response = authenticated_test_client.get(
        f"/plat_ing/restaurant/plat/{id_restaurant}"
    )
...
def test_add_ingredient_for_plat(
    ...
    response = authenticated_test_client.post(
        f"/plat_ing/plat/{plat_id}/ingredient/{ingredient_id}"
    )
    assert response.status_code == 200

    response = authenticated_test_client.post(
        f"/plat_ing/plat/-100/ingredient/{ingredient_id}"
    )
    assert response.status_code == 404

    response = authenticated_test_client.post(
        f"/plat_ing/plat/{plat_id}/ingredient/-100"
    )
    assert response.status_code == 404
